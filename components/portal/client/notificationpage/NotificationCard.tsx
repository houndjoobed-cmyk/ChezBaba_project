...
export default function NotificationCard({
  ...
  // Détecter si c'est une notification de remboursement à collecter
  const isRefundUrl =
    notification.urlRedirection?.startsWith("/client/refund?commandeId=") ?? false;

  const refundCommandeId = isRefundUrl
    ? new URLSearchParams(
        notification.urlRedirection!.replace("/client/refund?", "")
      ).get("commandeId") ?? ""
    : "";
  ...
  return (
    <>
                ...
                {notification.urlRedirection && (
                  isRefundUrl ? (
                    <button
                      ...
                      className="flex items-center gap-1 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg bg-green-100 hover:bg-green-200 text-green-800 transition-all duration-300 cursor-pointer text-sm sm:text-base font-medium"
                    >
                      <Eye className="h-3 w-3 sm:h-4 sm:w-4" />
                      <span>Fournir mes infos</span>
                    </button>
                  ) : (
      ...
      {/* Modale de collecte d'infos de remboursement */}
      {isRefundUrl && (
        <RefundInfoModal
          commandeId={refundCommandeId}
